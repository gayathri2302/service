...
router.get('/:projectId/mrs', async (req, res) => {
  try {
    ...
    res.json(await client(req.params.projectId).listMRs(state, page, perPage, search));
...
router.get('/:projectId/commits/:sha/diff', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).getCommitDiff(req.params.sha));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.get('/:projectId/commits/:sha', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).getCommitDetail(req.params.sha));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.post('/:projectId/mrs/:id/approve', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).approveMR(Number(req.params.id)));
  } catch (e: any) { res.status(500).json({ error: e.response?.data?.message || e.message }); }
});

router.post('/:projectId/mrs/:id/unapprove', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).unapproveMR(Number(req.params.id)));
  } catch (e: any) { res.status(500).json({ error: e.response?.data?.message || e.message }); }
});
...
router.get('/:projectId/mrs/:id/discussions', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).getMRDiscussions(Number(req.params.id)));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.post('/:projectId/mrs/:id/discussions/:discussionId/reply', async (req, res) => {
  try {
    res.json(await client(req.params.projectId).createDiscussionReply(Number(req.params.id), req.params.discussionId, req.body.body));
  } catch (e: any) { res.status(500).json({ error: e.response?.data?.message || e.message }); }
});
